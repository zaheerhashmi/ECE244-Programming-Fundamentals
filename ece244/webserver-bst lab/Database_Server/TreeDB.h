...
private:
   TreeNode* root;
   int probesCount;
   void delete_database (TreeNode* dbroot);
   DBentry* search_db (TreeNode* root, string key); 
   void insert_newEntry(TreeNode* root, DBentry* newEntry); 
   void activeCounter (TreeNode* root, int& count) const;
   void inorder(const TreeDB& tree);
   TreeNode* findparent(TreeNode* root,string name);
   TreeNode* max_in_lst(TreeNode* parent);
