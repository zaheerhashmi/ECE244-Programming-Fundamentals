...
AsteroidListItem* AsteroidList::insertAfter(AsteroidListItem* prev, const AsteroidList& others) { //gotta do//
   ...
   { AsteroidListItem* ptr = prev->getNext(); //storing the item after prev item
    ...
    //we would like to attach the first item of the copy of others to prev and the last item of the copy of others to the item following 
...
AsteroidList& AsteroidList::operator=(const AsteroidList& src) {
  // we are trying to assign new nodes to the lhs aka obj on which method is invoked so clear it.
    this->clear();
  // create a copy of src
    AsteroidList* tmp = new AsteroidList(src);
   // now we will attach the nodes of the src cpy to our lhs//
   this->head.setNext(tmp->begin()); // the head ptr of lhs now points to the nodes of copy src.
   delete tmp;
